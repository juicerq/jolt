      ...
      <form id="trigger-editor" className="flex flex-col gap-8" onSubmit={handleSubmit}>
        <fieldset className="m-0 flex min-w-0 flex-col gap-8 border-0 p-0 disabled:opacity-60" disabled={saving}>
          ...
          <SettingsSection title="Quando chamar o Bot">
            ...
            <Field label="Labels necessárias" optional><input className={fieldControlClassName} placeholder="bug, prioridade-alta" value={draft.labels} onChange={(event) => setDraft({ ...draft, labels: event.target.value })} /><span className="text-support font-normal text-muted">Separe por vírgulas. O evento precisa ter todas as labels indicadas; deixe vazio para aceitar qualquer uma.</span></Field>
            <div className="flex items-center justify-between gap-4"><div><p id="trigger-own-label" className="m-0 text-control text-secondary">Incluir eventos gerados pelo Mimo</p><p id="trigger-own-help" className="m-0 mt-1 text-support text-muted">O Bot também será chamado por ações que o Mimo fizer no GitHub.</p></div><Switch aria-labelledby="trigger-own-label" aria-describedby="trigger-own-help" checked={draft.includeOwnEvents} onChange={(includeOwnEvents) => setDraft({ ...draft, includeOwnEvents })} /></div>
          </SettingsSection>
        </fieldset>
      </form>
    </BotPage>
