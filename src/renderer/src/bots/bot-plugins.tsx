        ...
        <div className="flex flex-col gap-2" key={plugin.id}>
          ...
          <ul className="m-0 flex list-none flex-col divide-y divide-outline p-0">
            {plugin.accounts.map((account) => {
              ...
              return <PluginAccountRow key={account.id} account={account} actions={<Switch checked={granted} disabled={busy} aria-label={`Usar ${account.label}`} onChange={(checked) => onGrant(account.id, checked)} />} />
            })}
          </ul>
        </div>
      ))}
