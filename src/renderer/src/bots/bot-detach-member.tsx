...
export function BotDetachMember({ bot, leader, client }: { bot: Bot; leader: Bot; client: EngineClient }) {
  ...
  return (
    <SettingsSection title="Vínculo com o time">
      ...
      <Button className="self-start" variant="secondary" type="button" onClick={() => setConfirming(true)}>Desvincular do time</Button>
      {confirming && <BotDetachDialog bot={bot} leader={leader} client={client} onClose={() => setConfirming(false)} />}
    </SettingsSection>
  )
}

export function BotDetachDialog({ bot, leader, client, onClose }: { bot: Bot; leader: Bot; client: EngineClient; onClose: () => void }) {
  const queryClient = useQueryClient()
  const { mutate: detach, isPending, error } = useMutation(client.query.bots.detachMember.mutationOptions({
    async onSuccess() {
      onClose()
  ...
  return (
    <ConfirmationDialog
      ...
      actions={<>
        <Button variant="text" type="button" autoFocus disabled={isPending} onClick={onClose}>Cancelar</Button>
