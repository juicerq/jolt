...
export function ChatScroller({ children }: PropsWithChildren) {
  ...
  return (
    <div className="relative col-start-1 row-start-1 min-h-0 min-w-0">
      <div className="flex h-full min-h-0 max-h-none flex-col gap-0 overflow-y-auto p-0" ref={attachViewport} onScroll={handleScroll} aria-live="polite">
        <div className="box-border flex min-h-full flex-none flex-col gap-8 px-[clamp(28px,12vw,180px)] pt-16 pb-28 max-[700px]:px-5" ref={attachContent}>{children}</div>
      </div>
