...
function ConversationSidebar({ sessions, messages, selectedBotId, onCreate, onSelect }: { sessions: PrototypeSession[]; messages: Record<string, PrototypeMessage[]>; selectedBotId: string; onCreate: () => void; onSelect: (id: string) => void }) {
  return (
    <aside className="conversation-sidebar">
      <div className="conversation-sidebar-heading"><p>Bots</p><button type="button" aria-label="Criar bot" onClick={onCreate}><PlusIcon aria-hidden="true" /></button></div>
...
function ChatPanel({ bot, messages, draft, isRunning, onDraftChange, onOpenBot, onSend, onShowFunction, onStop }: { bot: PrototypeBot; messages: PrototypeMessage[]; draft: string; isRunning: boolean; onDraftChange: (value: string) => void; onOpenBot: (id: string) => void; onSend: () => void; onShowFunction: () => void; onStop: () => void }) {
  ...
  return (
    <section className="chat-panel">
      <button className="chat-settings-button" type="button" aria-label={`Abrir configurações de ${bot.name}`} onClick={onShowFunction}><Cog6ToothIcon aria-hidden="true" /></button>
      ...
      <div className="prompt-bar"><label htmlFor={`prompt-${bot.id}`}>Mensagem para {bot.name}</label><button className="prompt-add-button" type="button" aria-label="Adicionar contexto"><PlusIcon aria-hidden="true" /></button><input id={`prompt-${bot.id}`} placeholder={`Converse com ${bot.name}...`} value={draft} onChange={(event) => onDraftChange(event.target.value)} onKeyDown={handleComposerKey} />{isRunning ? <button className="stop-button" type="button" onClick={onStop} aria-label="Interromper resposta"><StopIcon aria-hidden="true" /></button> : <button className="prompt-send-button" type="button" disabled={!draft.trim()} onClick={onSend} aria-label="Enviar mensagem"><ArrowUpIcon aria-hidden="true" /></button>}</div>
    </section>
  )
}
...
function Message({ message, onOpenBot }: { message: PrototypeMessage; onOpenBot: (id: string) => void }) {
  ...
  return (
    <article className={`chat-message ${message.role}`}><div className="message-meta"><strong>{message.author}</strong><span>{message.time}</span></div>{message.activity && <Thinking activity={message.activity} running={message.status === "streaming"} />}<p>{message.content}{message.status === "streaming" && <span className="stream-cursor" aria-hidden="true" />}</p>
      {message.delegation && <div className={`delegation-card ${message.delegation.status}`}><span>{message.delegation.status === "done" ? <CheckIcon aria-hidden="true" /> : <ArrowTopRightOnSquareIcon aria-hidden="true" />}</span><div><strong>{message.delegation.bot}</strong><p>{message.delegation.task}</p></div><button type="button" onClick={() => onOpenBot(message.delegation?.botId ?? "leader")}>Abrir conversa</button></div>}
...
function BotCreator({ onClose, onSubmit }: { onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="prototype-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="member-creator" onSubmit={onSubmit}><div className="modal-heading"><div><p className="eyebrow">Novo chat</p><h2>Criar bot</h2></div><button type="button" onClick={onClose} aria-label="Fechar"><XMarkIcon aria-hidden="true" /></button></div><label>Nome<input name="name" placeholder="Ex: Sofia" autoFocus required /></label><label>Função<input name="outcome" placeholder="Ex: Revisar propostas antes do envio" required /></label><label>Executor<select name="provider" defaultValue="Claude Code"><option>Claude Code</option><option>Codex</option></select></label><WorkingPathField /><div className="modal-actions"><button type="button" onClick={onClose}>Cancelar</button><button type="submit">Criar bot</button></div></form></div>
}
...
function MemberCreator({ leader, onClose, onSubmit }: { leader: PrototypeBot; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="prototype-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="member-creator" onSubmit={onSubmit}><div className="modal-heading"><div><p className="eyebrow">Time de {leader.name}</p><h2>Novo integrante</h2></div><button type="button" onClick={onClose} aria-label="Fechar"><XMarkIcon aria-hidden="true" /></button></div><label>Nome<input name="name" placeholder="Ex: Sofia" autoFocus required /></label><label>Função<input name="outcome" placeholder="Ex: Analisar propostas antes do envio" required /></label><label>Executor<select name="provider" defaultValue="Claude Code"><option>Claude Code</option><option>Codex</option></select></label><WorkingPathField defaultValue={leader.workingPath} inheritedFrom={leader.name} /><label className="temporary-toggle"><input name="temporary" type="checkbox" /><span><strong>Integrante temporário</strong><small>{leader.name} pode removê-lo quando o trabalho terminar.</small></span></label><div className="modal-actions"><button type="button" onClick={onClose}>Cancelar</button><button type="submit">Criar integrante</button></div></form></div>
}
