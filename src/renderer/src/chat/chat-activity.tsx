...
function ActivityStageHeading({ step, status, details, prose, inlineDetail }: { step: VisibleStep; status: StageStatus; details: string[]; prose: boolean; inlineDetail: boolean }) {
  ...
  return (
    <>
      ...
      {details.length > 1 && <ChevronDownIcon className="mt-px size-[13px] opacity-0 transition-[opacity,transform] duration-150 ease-out group-hover/stage:opacity-100 group-focus-within/stage:opacity-100 group-open/stage:rotate-180 motion-reduce:transition-none" aria-hidden="true" />}
    </>
  )
}
