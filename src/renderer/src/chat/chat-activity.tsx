  ...
  return (
    <div className="group mb-4 w-fit text-support text-muted">
      <details onClick={blurMouseClick} className="group/activity max-w-[min(620px,100%)] transition-[opacity,transform] duration-180 ease-out starting:translate-y-1 starting:opacity-0 motion-reduce:transition-none">
        ...
        {steps.length === 1
          ? <div className={`${chatGuideClassName} mt-1.5 mb-1 ml-[14px] grid py-1 pl-4`}><SoloStage step={onlyStep} /></div>
          : (
...
function SoloStage({ step }: { step: VisibleStep }) {
  ...
  const { items, prose } = getChatActivityStepDetails(step)
      ...
      {details.length > 1
        ? (
            <details onClick={blurMouseClick} className="group/stage" open={expanded}>
              ...
              <ActivityDetailList className="ml-[23px]" details={details} prose={prose} />
            </details>
          )
        : <div className="grid min-w-0 grid-cols-[15px_minmax(0,1fr)] items-start gap-2">{heading}</div>}
      {expanded && step.type === "thinking" && step.content.trim() && <ThinkingTrace className={mode === "history" ? "ml-[23px] pt-0.5" : `${chatGuideClassName} ml-[23px] pl-4`} content={step.content} />}
      {expanded && details.length === 1 && <ActivityDetail className={`ml-[23px] block ${mode === "history" ? "pt-0.5" : chatGuideClassName + " py-1 pl-4"}`} prose={prose}>{details[0]}</ActivityDetail>}
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div className="flex min-w-0 flex-wrap gap-x-2 gap-y-1">
        ...
        {inlineDetail && details.length === 1 && <ActivityDetail prose={prose}>{details[0]}</ActivityDetail>}
      </div>
      {details.length > 1 && <ChevronDownIcon className="mt-px size-[13px] opacity-0 transition-[opacity,transform] duration-150 ease-out group-hover/stage:opacity-100 group-focus-within/stage:opacity-100 group-open/stage:rotate-180 max-md:opacity-100 motion-reduce:transition-none" aria-hidden="true" />}
    </>
  )
}

function ActivityDetailList({ className = "", details, prose }: { className?: string; details: string[]; prose: boolean }) {
  return (
    <ul className={`${className} m-0 grid list-none gap-0 p-0`}>
      {details.map((detail) => <li className="m-0 block min-w-0 border-0 p-0" key={detail}><ActivityDetail prose={prose}>{detail}</ActivityDetail></li>)}
    </ul>
  )
}
...
function ActivityDetail({ children, className = "", prose }: { children: string; className?: string; prose: boolean }) {
  if (prose) {
    return <span className={`${className} text-support text-muted`}>{children}</span>
  }

  return <code className={`${className} font-mono text-metadata text-muted [overflow-wrap:anywhere] whitespace-pre-wrap`}>{children}</code>
}
