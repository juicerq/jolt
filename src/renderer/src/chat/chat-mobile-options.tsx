...
export function ChatMobileOptions({ bot, client, disabled }: { bot: Bot; client: EngineClient; disabled: boolean }) {
  ...
  const rows = [
    { page: "model", label: "Modelo", value: currentModel?.name ?? currentModelId ?? "Padrão do Bot" },
    { page: "effort", label: "Esforço", value: effortLabels[bot.effort] },
  ...
  return <>
    <IconButton type="button" label="Opções da conversa" onClick={() => setPage("options")}><AdjustmentsHorizontalIcon /></IconButton>
    {execution.error && <p className="col-span-full m-0 px-2 text-support text-status-error" role="alert">Não foi possível alterar o ajuste: {execution.error.message}</p>}
    {page && <Dialog eyebrow={bot.name} title={pageTitles[page]} onClose={close}><DialogBody>
      ...
      {page === "options" && <div className="flex flex-col divide-y divide-outline">{rows.map((row) => <button type="button" key={row.page} className="flex min-h-16 items-center gap-3 rounded-lg py-3 text-left text-control text-primary focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:opacity-50" disabled={disabled || execution.isPending} onClick={() => setPage(row.page)}><span>{row.label}</span><span className="ml-auto min-w-0 truncate text-support text-secondary">{row.value}</span><ChevronRightIcon className="size-4 shrink-0 text-muted" /></button>)}</div>}
      {disabled && <p className="m-0 text-support text-secondary">Esses ajustes ficam disponíveis quando o Bot estiver parado e o computador conectado.</p>}
      {!disabled && page === "model" && <ChatModelOptions bot={bot} client={client} execution={execution} autoFocusSearch={false} onChoose={() => setPage("effort")} />}
      {!disabled && page === "effort" && <ChatEffortOptions bot={bot} execution={execution} onChoose={close} />}
      {!disabled && page === "permission" && <ChatPermissionOptions bot={bot} execution={execution} onChoose={close} />}
    </DialogBody></Dialog>}
  </>
}
