...
/** A popover nested inside another one, beside the row that opens it. Below `md` the mobile CSS turns it into a sheet like its parent. */
export const chatControlSubmenuClassName = `${menuCardClassName} chat-control-popover inset-auto mx-1 [position-area:right_span-top] [position-try-fallbacks:flip-inline,flip-block,flip-inline_flip-block]`
