...
const openClassName = "grid-cols-[20px_45px] [--edge-tab-open:1]"
...
export function ChatEdgeTab({ children }: { children: ReactNode }) {
  ...
  return (
    <div className={`absolute top-1/2 right-0 z-2 grid -translate-y-1/2 grid-cols-[20px_0px] items-center transition-[grid-template-columns] duration-150 ease-out [--edge-tab-open:0] motion-reduce:transition-none ${open ? openClassName : ""}`}>
      ...
      <div className={`${plateClassName} min-w-0 overflow-hidden opacity-[var(--edge-tab-open)] [&:has(>div>button:only-of-type)]:rounded-l-none transition-opacity duration-150 ease-out motion-reduce:transition-none`}>
