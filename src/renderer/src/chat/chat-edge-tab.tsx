...
/** Desktop only. On mobile the workspace top bar owns these actions. */
export function ChatEdgeTab({ children }: { children: ReactNode }) {
  ...
  return (
    <div className={`absolute top-1/2 right-0 z-2 grid -translate-y-1/2 grid-cols-[20px_0px] items-center transition-[grid-template-columns] duration-150 ease-out [--edge-tab-open:0] [--edge-tab-duration:150ms] [--edge-tab-ease:ease-out] [--edge-tab-outline:var(--color-outline)] [--edge-tab-step:steps(1,jump-end)] max-md:hidden motion-reduce:transition-none ${open ? openClassName : ""}`}>
