...
import type { CurationModel, CurationSettings } from "@src/shared/memory"
...
export function MemorySettings({ client }: { client: EngineClient }) {
  ...
  const unavailable = !!data && isModelUnavailable(data)
...
function modelChoices(settings: Pick<CurationSettings, "providers">) {
  return settings.providers.flatMap((catalog) => catalog.models.map((model) => ({ provider: catalog.provider, model: model.id })))
}

function isModelUnavailable(settings: CurationSettings) {
  return !!settings.model && !modelChoices(settings).some((choice) => modelValue(choice) === modelValue(settings.model))
}

function CurationModelSelect({ disabled, onChange, settings, unavailable }: { disabled: boolean; onChange: (model: CurationModel) => void; settings?: CurationSettings; unavailable: boolean }) {
  const choices = settings ? modelChoices(settings) : []

  return (
    <Select value={modelValue(settings?.model ?? null)} disabled={disabled} onChange={(event) => onChange(choices.find((choice) => modelValue(choice) === event.target.value) ?? null)}>
      <option value="">Usar o modelo de cada Bot</option>
      {unavailable && settings?.model && <option value={modelValue(settings.model)} disabled>{settings.model.model} · Indisponível</option>}
      {settings?.providers.map((catalog) => <optgroup key={catalog.provider} label={catalog.name}>
