...
function normalizeEvent(event: AgentSessionEvent): PiRuntimeEvent | undefined {
  ...
  if (event.type === "message_end" && event.message.role === "assistant") {
    ...
    if (reason === "stop" || reason === "aborted" || reason === "error") {
...
export async function createPiSessionFactory(options: { agentDirectory: string; sessionsDirectory: string; modelId: string }): Promise<PiSessionFactory> {
