...
export async function seedLoadDatabase(userDataDirectory: string, seed = 1) {
  ...
  function personTurn(botId: string) {
    ...
    append({ botId, author, authorBotId: null, taskId: null, content: pick(random, personPrompts), images, activity: null, ending: null })
    append({ botId, author: "bot", authorBotId: botId, taskId: null, content: botMarkdown(random), images: [], activity: botActivity(random), ending: turnEnding(random) })
  }
  ...
  function delegatedTurn(leader: StoredBot, member: StoredBot) {
    ...
    append({ botId: member.id, author: "bot", authorBotId: member.id, taskId: task.id, content: botMarkdown(random), images: [], activity: botActivity(random), ending: null })
    append({ botId: leader.id, author: "bot", authorBotId: member.id, taskId: task.id, content: botMarkdown(random), images: [], activity: null, ending: null })
