...
export function openRelayDatabase(path: string, secrets: RelaySecrets) {
  ...
  const completeConnectionStatement = database.prepare("UPDATE connections SET status = 'connected', installation_id = ?, account_login = ?, relay_token_hash = ?, relay_token_sealed = ? WHERE id = ?")
  const authorizeInstallationStatement = database.prepare<ConnectionRow, [string, string]>("SELECT * FROM connections WHERE installation_id = ? AND relay_token_hash = ? AND status = 'connected' ORDER BY created_at DESC LIMIT 1")
