...
describe("engine router", () => {
  ...
  test("a failure before the Bot replies reaches the Renderer with its own message", async () => {
    ...
    const stream = await environment.client.conversations.send({ botId: bot.id, content: "oi" })

    expect(stream.next()).rejects.toThrow("Working directory is not accessible")
    await environment.close()
  })
})
