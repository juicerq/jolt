...
const bot: Bot = { id: "b1", leaderBotId: null, projectId: null, name: "Testador", provider: "codex", function: { outcome: "Testes passando" }, workingDirectoryOverride: null, temporary: false, memoryEnabled: true, effort: "medium", model: null, createdAt: "2026-09-01T12:00:00.000Z", effectiveWorkingDirectory: "/tmp/b1", closed: false }
...
describe("Bot settings", () => {
  ...
  test("a change trims the text and maps empty choices to null", () => {
    expect(settingsChange(bot, { ...draftOf(bot), name: " Revisor ", description: "  " })).toEqual({
      complete: true,
      input: { id: "b1", name: "Revisor", function: { outcome: "Testes passando" }, projectId: null, workingDirectoryOverride: null, memoryEnabled: true, effort: "medium", model: null },
    })
