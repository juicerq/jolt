...
describe("Bot settings", () => {
  ...
  test("a change trims the text and maps empty choices to null", () => {
    expect(settingsChange(bot, { ...draftOf(bot), name: " Revisor ", description: "  " })).toEqual({
      complete: true,
      input: { id: "b1", name: "Revisor", function: { outcome: "Testes passando" }, projectId: null, workingDirectoryOverride: null, memoryEnabled: true, effort: "medium", model: null, permissionMode: "ask" },
    })
